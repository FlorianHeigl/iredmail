...
disable_rhms_repo()
{
    ...
    ECHO_INFO "Disable yum repo generated by rhms." && \
    perl -pi -e 's#^(enabled.*=)1#${1}0#' /etc/yum.repos.d/${LOCAL_REPO_NAME}.repo

    echo 'export status_disable_rhms_repo="DONE"' >> ${STATUS_FILE}
}

replace_iptables_rule()
{
    ECHO_QUESTION "Would you like to use iptables rules shipped within rhms now?"
...
replace_mysql_config()
{
    if [ X"${BACKEND}" == X"MySQL" ]; then
        ECHO_QUESTION "Would you like to use MySQL configuration file shipped within rhms now?"
...
EOF
    ...
    check_status_before_run disable_rhms_repo
